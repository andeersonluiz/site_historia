...
class DiffMatchPatch {
  ...
  List<Diff> diff_main(String? text1, String? text2,
    ...
    if (deadline == null) {
      ...
      if (Diff_Timeout <= 0) {
        ...
        deadline =
            deadline.add(Duration(milliseconds: (Diff_Timeout * 1000).toInt()));
      }
    }
  ...
  Map<String, dynamic> _diff_linesToChars(String text1, String text2) {
    ...
    var chars1 = _diff_linesToCharsMunge(text1, lineArray, lineHash, 40000);
    var chars2 = _diff_linesToCharsMunge(text2, lineArray, lineHash, 65535);
  ...
  void diff_cleanupSemantic(List<Diff> diffs) {
    ...
    while (pointer < diffs.length) {
      if (diffs[pointer - 1].operation == Operation.delete &&
        ...
        var overlap_length1 = _diff_commonOverlap(deletion, insertion);
        var overlap_length2 = _diff_commonOverlap(insertion, deletion);
  ...
  void _diff_cleanupSemanticLossless(List<Diff> diffs) {
    ...
    int _diff_cleanupSemanticScore(String one, String? two) {
      ...
      var char2 = two[0];
      var nonAlphaNumeric1 = char1.contains(nonAlphaNumericRegex_);
      var nonAlphaNumeric2 = char2.contains(nonAlphaNumericRegex_);
      var whitespace1 = nonAlphaNumeric1 && char1.contains(whitespaceRegex_);
      var whitespace2 = nonAlphaNumeric2 && char2.contains(whitespaceRegex_);
      var lineBreak1 = whitespace1 && char1.contains(linebreakRegex_);
      var lineBreak2 = whitespace2 && char2.contains(linebreakRegex_);
      var blankLine1 = lineBreak1 && one.contains(blanklineEndRegex_);
      var blankLine2 = lineBreak2 && two.contains(blanklineStartRegex_);
    ...
    while (pointer < diffs.length - 1) {
      if (diffs[pointer - 1].operation == Operation.equal &&
        ...
        var bestScore = _diff_cleanupSemanticScore(equality1, edit) +
        ...
        while (edit.isNotEmpty &&
          ...
          var score = _diff_cleanupSemanticScore(equality1, edit) +
              _diff_cleanupSemanticScore(edit, equality2);
  ...
  int _match_bitap(String text, String pattern, int loc) {
    ...
    for (var d = 0; d < pattern.length; d++) {
      ...
      for (var j = finish; j >= start; j--) {
        ...
        if ((rd[j] & matchmask) != 0) {
          var score = _match_bitapScore(d, j - 1, loc, pattern);
      ...
      while (bigpatch.diffs!.isNotEmpty) {
        ...
        while (bigpatch.diffs!.isNotEmpty &&
          ...
          if (diff_type == Operation.insert) {
            ...
            start2 = start2! + diff_text.length;
            ...
            start1 = start1! + diff_text.length;
            empty = false;
